...
area_ha = property_aea[['geometry']].geometry.area.sum() / 10000

codempreendimento_df = pd.read_csv('https://www.bcb.gov.br/htms/sicor/Empreendimento.csv', encoding='windows-1252')

empreendimento_df = codempreendimento_df[codempreendimento_df['#CODIGO']==12010560176011]
...
dados_cadastrais_df = {
    ...
    'Expectativa de produtividade (Kg/ha)': (gdf['Grupo_COR0001_DestcFincmnt_QtdPrvProdc'].astype(float).sum()/1000)/area_ha,
    'Expectativa de produção (ton)': gdf['Grupo_COR0001_DestcFincmnt_QtdPrvProdc'].astype(float).sum(),
    'Receita esperada do empreendimento (BRL)':gdf['Grupo_COR0001_DestcFincmnt_VlrReceitaBrutEsprdEmpnmnt'].astype(float).sum()
}
