...
if "destinations" in parsed_data:
    for i, dest in enumerate(parsed_data["destinations"]):
        ...
        if dest["properties"]:
            display(Markdown("Imóveis Associados"))
            ...
            display(Markdown(f'{markdown_table_matr}\n\n: Imóveis Associados ao Empreendimento {i+1}'))
```
...
"""
    ...
    if total_rasters > 0:
        colunas = 4
        # Calcule o número de linhas necessárias para as imagens
        linhas_de_imagens = math.ceil(total_rasters / colunas)
        # A altura da figura será o número de linhas de imagens vezes 2 (para RGB + EVI)
        fig, axes = plt.subplots(
            nrows=linhas_de_imagens * 2, 
            ncols=colunas, 
            figsize=(16, 6 * linhas_de_imagens),
            gridspec_kw={'hspace': 0.3, 'wspace': 0.1}
        )
        # Achata o array de eixos para facilitar a iteração se houver mais de uma linha/coluna
        axes = axes.flatten()
        
        # Esconde eixos extras que não serão usados
        for i in range(total_rasters * 2, len(axes)):
            axes[i].set_axis_off()

        # Crie o mapa de cores de Vermelho para Verde
        from matplotlib.colors import LinearSegmentedColormap
        colors = [(1, 0, 0), (1, 1, 0), (0, 1, 0)] # R, Y, G
        cmap_name = 'RdYlGn_custom'
        ...
        for item in time_ordered_items:
            ...
            ax_rgb = axes[t * 2]
            try:
                banda_04_rgb = carregar_banda_sentinel2_bdc(item, 'B04', gleba_geo)
                if banda_04_rgb.size == 0: 
                    ax_rgb.set_title(f"{title}\n(RGB Erro)"); ax_rgb.set_xticks([]); ax_rgb.set_yticks([])
                    # Pula para o próximo item se a banda principal estiver vazia
                    t += 1
                    continue

                banda_08_rgb = carregar_banda_sentinel2_bdc(item, 'B08', gleba_geo)
                banda_11_rgb = carregar_banda_sentinel2_bdc(item, 'B11', gleba_geo)
                banda_11_zoom = resize(banda_11_rgb, banda_04_rgb.shape, order=1, preserve_range=True, anti_aliasing=True)
                
                L, N = 2**12, 2
                matriz_rgb = np.zeros((banda_04_rgb.shape[0], banda_04_rgb.shape[1], 3), dtype=np.float32)
                matriz_rgb[:, :, 0] = aplicar_contraste_raiz(normalizar(banda_08_rgb, L), L, N) / L
                matriz_rgb[:, :, 1] = aplicar_contraste_raiz(normalizar(banda_11_zoom, L), L, N) / L
                matriz_rgb[:, :, 2] = aplicar_contraste_raiz(normalizar(banda_04_rgb, L), L, N) / L
                
                ax_rgb.imshow(np.clip(matriz_rgb, 0, 1))
                ax_rgb.set_title(f"{title} (RGB)")
            ...
            try:
                ...
                if banda_04_ref.size == 0:
                    ax_evi.set_title(f"{title}\n(EVI Erro)"); ax_evi.set_xticks([]); ax_evi.set_yticks([])
                    t += 1
                    continue
                ...
                im = ax_evi.imshow(banda_evi, cmap=cm, vmin=0, vmax=1)
                ax_evi.set_title(f"{title} (EVI)")
                ax_evi.set_xticks([]); ax_evi.set_yticks([])
        ...
        # Adiciona uma única colorbar para o EVI
        fig.subplots_adjust(right=0.9)
        cbar_ax = fig.add_axes([0.92, 0.15, 0.02, 0.7])
        fig.colorbar(im, cax=cbar_ax, label='EVI')
