...
def parse_operation_details_from_xml(xml_string):
    """
    Parses a COR0001 XML string to extract structured details about
    financing destinations and associated properties (matricules).
    """
    ...
    if cor_element is None:
        return {"error": "Elemento COR0001 não encontrado no XML."}
 
    def find_text(element, path):
        node = element.find(path, ns)
        return node.text.strip() if node is not None and node.text else "N/A"
 
    general_info = {
        "Número de Controle IF": find_text(cor_element, 'ns:NumCtrlIF'),
        "CNPJ da Entidade Responsável": find_text(cor_element, 'ns:CNPJEntRespons'),
        "Data de Emissão": find_text(cor_element, 'ns:DtEms'),
        "Data de Vencimento": find_text(cor_element, 'ns:DtVenc'),
        "Valor Total da Operação": find_text(cor_element, 'ns:VlrTotOp'),
        "Data do Movimento": find_text(cor_element, 'ns:DtMovto'),
    }
 
    destinations = []
    for dest_elem in cor_element.findall('ns:Grupo_COR0001_DestcFincmnt', ns):
        dest_info = {
            "Código do Empreendimento": find_text(dest_elem, 'ns:CodEmpnmnt'),
            "Valor da Parcela de Crédito": find_text(dest_elem, 'ns:VlrParclCred'),
            "Início do Plantio": find_text(dest_elem, 'ns:DtIniPlant'),
            "Fim do Plantio": find_text(dest_elem, 'ns:DtFimPlant'),
            "Início da Colheita": find_text(dest_elem, 'ns:DtIniColht'),
            "Fim da Colheita": find_text(dest_elem, 'ns:DtFimColht'),
            "Receita Bruta Esperada": find_text(dest_elem, 'ns:VlrReceitaBrutEsprdEmpnmnt'),
        }
 
        properties = []
        for prop_elem in dest_elem.findall('ns:Grupo_COR0001_Propt', ns):
            prop_details = {
                "NIRF": find_text(prop_elem, 'ns:NIRF'),
                "Código SNCR": find_text(prop_elem, 'ns:CodSNCR'),
                "Número CAR": find_text(prop_elem, 'ns:NumRegCAR'),
            }
            properties.append(prop_details)
       
        dest_info["properties"] = properties
        destinations.append(dest_info)
 
    return {"general_info": general_info, "destinations": destinations}
...
def parse_xml_to_gdf_all_fields(xml_string):
    ...
    # Assuming recursive_extract extracts all fields from cor element except geometry
    data = recursive_extract(cor, ns)
   
    data_ = {x:data[x] for x in data if 'Grupo_COR0001_DestcFincmnt' not in x}
   
    polygon_points = []

    # Prepare list to hold rows for GeoDataFrame
    rows = []
   
    # Iterate over all Grupo_COR0001_DestcFincmnt elements (credit operations)
    for destc in cor.findall('ns:Grupo_COR0001_DestcFincmnt', ns):
        # Extract data for this credit operation (you may want to extract specific fields here)
        destc_data = recursive_extract(destc, ns)
       
        polygons = []
        # Iterate over all glebas (land parcels) inside this credit operation
        for gleba in destc.findall('ns:Grupo_COR0001_Gleba', ns):
            polygon_points = []
            for pt in gleba.findall('ns:Grupo_COR0001_PontoPolg', ns):
                lat = pt.find('ns:LatPonto', ns)
                lon = pt.find('ns:LongPonto', ns)
        ...
        if not polygons:
            # Skip if no polygons found for this credit operation
            continue
        ...
        destc_data.update(data_)

        # Combine destc_data and geometry into one dict for GeoDataFrame row
        row = destc_data.copy()
        row['geometry'] = geometry
        rows.append(row)
   
    if not rows:
        raise ValueError("No credit operations with land parcels found in XML")
   
    gdf = gpd.GeoDataFrame(rows, crs="EPSG:4326")
    return gdf
 
def retry_request(func, max_retries=3, delay=15, name="operação de rede", **kwargs):
    """
...
# Este bloco carrega e processa os dados do XML, mas não produz output visível.
# As variáveis `gdf` e `parsed_data` ficam disponíveis para os blocos seguintes.
try:
...
if xml_content:
    ...
    parsed_data = parse_operation_details_from_xml(xml_content)
else:
    gdf = None
    parsed_data = {}
```
...
dados_cadastrais_df = {
    ...
    'Produtor': produtor,
    'CPF / CNPJ': gdf['Grupo_COR0001_Benfcrio_CNPJ_CPFBenfcrio'].iloc[0],
    'REFBACEN': ref_bacen,
    #'Empreendimento': '{produto} | {variedade} | {cesta}'.format(
    #    produto=empreendimento_df['PRODUTO'].values[0],
    #    variedade=empreendimento_df['VARIEDADE'].values[0],
    #    cesta=empreendimento_df['CESTA'].values[0]
    #    ),
    'Área Financiada (ha)': area_ha,
    #'Expectativa de produtividade (Kg/ha)': (gdf['QtdPrvProdc'].astype(float).sum()/1000)/area_ha,
    #'Expectativa de produção (ton)': gdf['QtdPrvProdc'].astype(float).sum(),
    'Receita esperada do empreendimento (BRL)':gdf['VlrReceitaBrutEsprdEmpnmnt'].astype(float).sum()
}

# Convert dict to list of [attribute, value]
table_data = [[k, v] for k, v in dados_cadastrais_df.items()]
...
codempreendimento_df = pd.read_csv('https://www.bcb.gov.br/htms/sicor/Empreendimento.csv', encoding='windows-1252').rename(columns={'#CODIGO':'Código do Empreendimento'})

codempreendimento_df['Código do Empreendimento'] = codempreendimento_df['Código do Empreendimento'].astype(str)


parsed_data["destinations"] = pd.DataFrame(parsed_data["destinations"]).merge(codempreendimento_df, how='left', on='Código do Empreendimento').to_dict(orient='records')


if "destinations" in parsed_data:
    for i, dest in enumerate(parsed_data["destinations"]):
        display(Markdown(f"#### Empreendimento {i+1} (Código: {dest['Código do Empreendimento']})"))
        
        # Detalhes do empreendimento
        dest_details = [[k, v] for k, v in dest.items() if k != "properties"]
        markdown_table_dest = tabulate(dest_details, headers=["Atributo", "Valor"], tablefmt="pipe")
        display(Markdown(f'{markdown_table_dest}\n\n: Detalhes do Empreendimento {i+1}'))
        
        # Matrículas associadas
        if dest["properties"]:
            display(Markdown("##### Matrículas Associadas"))
            matr_data = [list(d.values()) for d in dest["properties"]]
            headers = list(dest["properties"][0].keys())
            markdown_table_matr = tabulate(matr_data, headers=headers, tablefmt="pipe")
            display(Markdown(f'{markdown_table_matr}\n\n: Matrículas Associadas ao Empreendimento {i+1}'))
```
...
def processar_gleba(gleba_row, gleba_id):
    """
    ...
    gleba_data['DtIniPlant'] = pd.to_datetime(gleba_data['DtIniPlant']) - pd.DateOffset(years=1)
    gleba_data['DtFimColht'] = pd.to_datetime(gleba_data['DtFimColht']) - pd.DateOffset(years=1)
    ...
    # Extrai as datas de início e fim dinamicamente
    ...
    print(f"## Período de Análise: {gleba_data['DtIniPlant'].strftime('%d/%m/%Y')} a {gleba_data['DtFimColht'].strftime('%d/%m/%Y')}\n")
...
"""
    ...
    print("A primeira etapa consiste na identificação da propriedade no território, sua localização em relação ao município e estado, e o cálculo de métricas espaciais básicas.\n")
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Mapa de localização da propriedade rural com informações detalhadas.*\n:::\n:::')
    fig_localization = plot_rural_property(joined, municipalities_gdf=municipio_filtrado)
    plt.show()

    print("\n### Análise de Uso e Cobertura do Solo\n")
    print("#### Cruzamento com MapBiomas\n")
    print("A gleba foi cruzada com os dados mais recentes do MapBiomas (Coleção 10) para identificar as classes de uso e cobertura do solo predominantes na área.\n")
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Classificação de uso e cobertura do solo na área da gleba segundo o MapBiomas.*\n:::\n:::')
    ...
    print("Para analisar a dinâmica da vegetação ao longo do tempo, foram gerados pontos de amostragem dentro da gleba. Para cada ponto, foram extraídas as séries temporais do índice de vegetação NDVI a partir de duas fontes de dados: Sentinel-2 (10m de resolução) e CBERS-WFI (64m de resolução).\n")
    print("#### Geração de Pontos de Amostragem\n")
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Pontos de amostragem distribuídos em grade sobre a gleba.*\n:::\n:::')
    ...
    gleba_gdf_plot = gpd.GeoDataFrame(geometry=[gleba_4326], crs="EPSG:4326")
    pontos_proj = pontos_gdf.to_crs(epsg=3857)
    gleba_proj = gleba_gdf_plot.to_crs(epsg=3857)
    ...
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Séries temporais de NDVI (Sentinel-2 e CBERS-WFI) para os pontos amostrados.*\n:::\n:::')
    
    # --- NEW: Function to wrap the entire WTSS initialization ---
    def initialize_and_fetch_points():
        # Step 1: Initialize the service and cubes (this is where the error occurred)
        ...
        # Step 2: Fetch time series for each point (this part was already correct)
        import matplotlib.gridspec as gridspec
        successful_plots = 0
        for i, ponto in enumerate(pontos_aleatorios):
            # This inner call still needs a simple try/except because it's inside the main one
            try:
                print_status(f"   - Ponto {i+1}...")
                ...
                if not ts_s2 or not ts_wfi:
                    print_status(f"   - AVISO: Dados de série temporal incompletos para o ponto {i+1}.")
                    continue
                ...
                print_status(f"   - AVISO: Erro ao buscar dados para o ponto {i+1}: {e}. Pulando.")
        
        # If after all points, none were successful, this is noted.
        if successful_plots == 0 and total_pontos > 0:
             print_status(f"   - AVISO FINAL: Nenhuma série temporal foi plotada com sucesso.")
    
    
    # --- MODIFIED: Call the new wrapped function within the retry handler ---
    _, error = retry_request(
        func=initialize_and_fetch_points,
        name="conexão inicial e busca de séries temporais (WTSS)"
    )
    
    if error:
        print('::: {.callout-warning}')
        print('## Falha na Obtenção das Séries Temporais')
        print(f'Não foi possível conectar ao serviço WTSS do INPE ou obter os dados após múltiplas tentativas. Esta seção da análise foi pulada. Erro: {error}')
        print(':::')

    print("\n### Análise de Imagens de Satélite\n")
    print("Foram processadas imagens do satélite Sentinel-2 para o período de análise. Para cada data disponível, foi gerada uma composição colorida e um gráfico de estatísticas.\n")
    print("#### Recortes de Imagens e Estatísticas Temporais\n")
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Recortes de imagens Sentinel-2 e estatísticas de cobertura para cada data.*\n:::\n:::')
    ...
    if error:
        print('::: {.callout-warning}')
        print('## Falha na Busca de Imagens de Satélite')
    ...
    if total_rasters > 0:
        ...
        for item in time_ordered_items:
            ...
            banda_scl = carregar_banda_sentinel2_bdc(item, 'SCL', gleba_geo)
            banda_04_ref = carregar_banda_sentinel2_bdc(item, 'B04', gleba_geo)
            banda_scl_zoom = resize(banda_scl, banda_04_ref.shape, order=0)
            banda_03 = carregar_banda_sentinel2_bdc_scl(item, 'B03', gleba_geo, banda_scl_zoom)
            banda_04 = carregar_banda_sentinel2_bdc_scl(item, 'B04', gleba_geo, banda_scl_zoom)
            banda_08 = carregar_banda_sentinel2_bdc_scl(item, 'B08', gleba_geo, banda_scl_zoom)
            banda_11 = carregar_banda_sentinel2_bdc(item, 'B11', gleba_geo)
    ...
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Composições coloridas (B08, B11, B04) sem máscara de nuvens.*\n:::\n:::')
    ...
    if total_rasters > 0:
        ...
        for item in time_ordered_items:
            ...
            title = item.properties['datetime'][:10]
            try:
                banda_04 = carregar_banda_sentinel2_bdc(item, 'B04', gleba_geo)
                if banda_04.size == 0: continue
                banda_08 = carregar_banda_sentinel2_bdc(item, 'B08', gleba_geo)
                banda_11 = carregar_banda_sentinel2_bdc(item, 'B11', gleba_geo)
    ...
    print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Mapas de EVI (Enhanced Vegetation Index) para cada data.*\n:::\n:::')
    ...
    if total_rasters > 0:
        ...
        for item in time_ordered_items:
            ...
            try:
                ...
                banda_02 = carregar_banda_sentinel2_bdc_scl(item, 'B02', gleba_geo, banda_scl_zoom)
                banda_04 = carregar_banda_sentinel2_bdc_scl(item, 'B04', gleba_geo, banda_scl_zoom)
                banda_08 = carregar_banda_sentinel2_bdc_scl(item, 'B08', gleba_geo, banda_scl_zoom)
    ...
    print("Utilizando a técnica de Mapas Auto-Organizáveis (SOM), as séries temporais de NDVI de cada pixel da gleba foram agrupadas em n x n clusters. Isso permite identificar áreas com comportamento fenológico similar ao longo do tempo (por exemplo, áreas de cultivo, pastagem, vegetação nativa).\n")
    print_status("9. Criando cubo de dados para clusterização SOM...")

    cubo, mask, error = None, None, None
    try:
        if total_rasters > 0: # Only try if we found images in the previous step
    ...
    if error or cubo is None:
        ...
        if error:
            print('::: {.callout-warning}')
            print('## Falha na Análise de Clusterização (SOM)')
        ...
        print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Análise combinada dos clusters SOM, mostrando os perfis temporais de NDVI (esquerda) e sua distribuição espacial na gleba (direita).*\n:::\n:::')
        ...
        print('::: {.cell}\n::: {.cell-output-display}\n*Fig. Matriz-U do SOM, mostrando a separabilidade entre os clusters.*\n:::\n:::')
...
# --- Bloco Principal do Relatório ---

# Explode o GeoDataFrame para garantir que cada polígono individual seja processado.
# exploded_gdf = gdf.explode(index_parts=True).iloc[[0]] # Remova o .iloc[[0]] para rodar para todas
exploded_gdf = gdf.explode(index_parts=True)

# Itera sobre cada gleba e chama a função de processamento
for index, gleba_row in exploded_gdf.iterrows():
    # O índice pode ser um tuple (se index_parts=True), então pegamos o ID original
    gleba_id = index[0] if isinstance(index, tuple) else index

    # Chama a função principal, que cuida de tudo
    processar_gleba(gleba_row, gleba_id)
